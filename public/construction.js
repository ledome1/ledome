document.head.insertAdjacentHTML("beforeend",'<link rel="stylesheet" href="/construction-overrides.css?v=141">');
...
function diaryOutsideCostBox(diaryDate){return `<section class="diary-outside-cost"><header><div><h4>PHIẾU CHI</h4><p>Khoản mua thêm ngoài kho, ghi nhận riêng theo ngày ${diaryDate}.</p></div><button type="button" data-form="expense">+ Phiếu chi</button></header><table><thead><tr><th>Mã phiếu</th><th>Nội dung</th><th>Đối tượng</th><th>Giá trị</th></tr></thead><tbody><tr><td><a data-view-link="cash">PC-00216</a></td><td>Sơn dặm hoàn thiện khu WC</td><td>NCC Minh An</td><td>12 kg</td></tr></tbody></table></section>`}
function diaryOwnerTicketBox(){const items=[["Yêu cầu Phát sinh của CDT","rfi"],["Yêu cầu của CDT","owner-request"],["Yêu cầu cần CDT phê duyệt","rfa"],["Vấn đề sự cố","issues"]];return `<section class="diary-owner-tickets"><header><h4>PHIẾU CDT / SỰ CỐ</h4></header><div>${items.map(([label,view])=>`<button type="button" data-view-link="${view}">+ Thêm ${label}</button>`).join("")}</div></section>`}
...
function diaryWarehouseItemOptions(){return [...new Set([...(Array.isArray(warehouseRows)?warehouseRows.map(row=>row[1]):[]),"Bột trét tường","Sơn ICI Dulux cao cấp Weather Shield ngoài nhà"].filter(Boolean))]}
...
function diaryTaskMaterials(open){const warehouseOptions=["Kho văn phòng","Kho dự án"],itemOptions=diaryWarehouseItemOptions();const rows=[["1",diaryEditSelect("Bột trét tường",itemOptions),diaryEditSelect("Kho dự án",warehouseOptions),diaryEditInput("18:00",'type="time"'),diaryEditInput("23",'inputmode="decimal"'),diaryEditSelect("hộp",diaryMaterialUnitOptions),diaryDeleteRowButton("materials")],["2",diaryEditSelect("Sơn ICI Dulux cao cấp Weather Shield ngoài nhà",itemOptions),diaryEditSelect("Kho dự án",warehouseOptions),diaryEditInput("18:00",'type="time"'),diaryEditInput("45",'inputmode="decimal"'),diaryEditSelect("kg",diaryMaterialUnitOptions),diaryDeleteRowButton("materials")]];return `<div class="diary-task-table-scroll" data-diary-section="materials" ${open?"":"hidden"}>${diaryEditableTable(["STT","Thiết bị Vật tư","Kho xuất","Thời gian xuất","Số lượng","Đơn vị",""],rows)}</div>`}
...
function diaryAddedRow(kind,index){return kind==="materials"?`<tr><td>${index}</td><td>${diaryEditSelect(diaryWarehouseItemOptions()[0]||"",diaryWarehouseItemOptions())}</td><td>${diaryEditSelect("Kho dự án",["Kho văn phòng","Kho dự án"])}</td><td>${diaryEditInput("18:00",'type="time"')}</td><td>${diaryEditInput("",'inputmode="decimal"')}</td><td>${diaryEditSelect("kg",diaryMaterialUnitOptions)}</td><td>${diaryDeleteRowButton("materials")}</td></tr>`:`<tr><td>${diaryEditInput(`Ca ${index}`)}</td><td>${diaryEditInput("",'inputmode="numeric"')}</td><td>${diaryEditInput("07:00",'type="time"')}</td><td>${diaryEditInput("18:00",'type="time"')}</td><td>${diaryDeleteRowButton("labor")}</td></tr>`}
