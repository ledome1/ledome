...
function ensureGanttTaskList(){
  const signature=`${currentProject?.id||id}:${projectScheduleStages().join("-")}:${isDemoProject()?"demo":"runtime"}`;
  if(ganttTaskList?.signature===signature)return ganttTaskList;
  let index=0;
  ganttTaskList=isDemoProject()?projectScheduleStages().flatMap(stage=>ganttStageTemplates[stage].map(([id,name,parentId="",isGroup=false])=>({id,name,parentId,isGroup,stage,baseIndex:index++}))):[];
