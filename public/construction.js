document.head.insertAdjacentHTML("beforeend",'<link rel="stylesheet" href="/construction-overrides.css?v=171">');
...
function projectInfoCard(p,progress,budget,spent){
  ...
  return `<section class="project-summary project-info project-info-inline box" data-project-info-inline>
    <header><div><h2>${esc(p.name)}</h2><p><i></i>${status} - ${health}</p></div><div class="project-info-actions">${canEdit?`<button data-project-info-delete class="danger">X\u00f3a d\u1ef1 \u00e1n</button><button data-project-info-save disabled>L\u01b0u th\u00f4ng tin</button><button data-project-info-edit>Thay \u0111\u1ed5i</button>`:`<span>Ch\u1ec9 xem</span>`}</div></header>
...
function diarySupplementalTasksBox(diaryDate){const rows=diarySupplementalTaskRows(diaryDate);return `<section class="diary-supplemental-tasks"><header><div><h4>DANH SÁCH NHIỆM VỤ BỔ SUNG</h4><p>Bao gồm giao thêm, yêu cầu ghi chú của CDT, chủ trì dự án và task không hoàn thành hôm trước.</p></div><span>${rows.length}</span></header><div>${rows.map(row=>`<article class="${row.type}"><b title="${contractEscape(row.title)}">${contractEscape(row.title)}</b><small>${contractEscape(row.source)}</small><footer><em>${contractEscape(row.owner)}</em><strong>${contractEscape(row.status)}</strong>${row.link?`<button type="button" data-view-link="${row.link}">Mở</button>`:""}</footer></article>`).join("")}</div></section>`}
...
function refreshDiarySupplementalTasks(diaryDate){const box=document.querySelector(".diary-supplemental-tasks");if(box)box.outerHTML=diarySupplementalTasksBox(diaryDate);document.querySelectorAll("[data-view-link]").forEach(x=>x.onclick=()=>showView(x.dataset.viewLink))}
...
function renderDiaryTaskCards(value){const content=document.querySelector("[data-diary-editor-content]");if(content){content.innerHTML=diaryTaskCards(value);bindDiaryTaskUploads(content);bindDiarySectionToggles(content);bindDiaryAddRows(content);bindDiaryDeleteRows(content);bindDiaryTaskResults(content);bindDiaryStandardLinks(content);bindDiaryStandardChecks(content);content.querySelectorAll("[data-view-link]").forEach(x=>x.onclick=()=>showView(x.dataset.viewLink))}}
