document.head.insertAdjacentHTML("beforeend",'<link rel="stylesheet" href="/construction-overrides.css?v=181">');
...
function expenseQuickList(total){
  ...
  const body=rows.length?rows.map(row=>`<tr><td><a data-form="expense">${row[0]}</a></td><td>${row[1]}</td><td>${row[2]}</td><td>${row[3]}</td><td>${fmt(row[4])}</td><td><span class="${row[5]==="Chờ duyệt"?"pending":"done"}">${row[5]}</span></td></tr>`).join(""):`<tr><td colspan="6">Chưa có phiếu chi nào được ghi nhận cho dự án này.</td></tr>`;
...
function projectChatAttachmentsMarkup(message){
  ...
  const body=attachments.map((file)=>{
    ...
    if(file.category==="image")return `<a class="project-chat-attachment image" href="${url}" target="_blank" rel="noopener"><img src="${url}" alt="${name}"><span>${name}<small>${size} · ${expires}</small></span></a>`;
    if(file.category==="video")return `<div class="project-chat-attachment video"><video src="${url}" controls preload="metadata"></video><a href="${url}" target="_blank" rel="noopener">${name}<small>${size} · ${expires}</small></a></div>`;
    return `<a class="project-chat-attachment file" href="${url}&download=1" target="_blank" rel="noopener"><b>↧</b><span>${name}<small>${size} · ${expires}</small></span></a>`;
  }).join("");
...
async function render(){
  ...
  ${projectChatPanel(p)}
  <section class="quick box"><h2>Truy cập nhanh</h2><div class="quick-grid quick-grid-three"><div><b>Cá nhân</b><a data-external-link="/attendance/">Thêm phiếu chấm công</a><a data-app-link="hrm-overtime">Thêm phiếu overtime</a><a data-app-link="hrm-leave">Thêm phiếu xin nghỉ phép</a><a data-view-link="gantt">Thêm phiếu nhiệm vụ</a></div><div><b>Công trường</b><a data-form="slip-in">Thêm phiếu nhập kho</a><a data-form="slip-out">Thêm phiếu xuất kho</a><a data-form="expense">Thêm phiếu chi</a></div><div><b>CDT</b><a data-view-link="rfi">Thêm phiếu Yêu cầu Phát sinh của CDT</a><a data-view-link="owner-request">Thêm phiếu Yêu cầu ghi chú của CDT</a><a data-view-link="rfa">Thêm phiếu Yêu cầu cần CDT phê duyệt</a><a data-view-link="issues">Thêm phiếu Vấn đề sự cố</a></div></div></section>
...
function showForm(type){
  ...
  if(type==="slip-in"||type==="slip-out"){const isIssue=type==="slip-out";return formShell(isIssue?"Tạo phiếu xuất kho":"Tạo phiếu nhập kho",`<div class="split-form"><section><div class="form-tabs"><input placeholder="⌕ Tìm hàng hóa theo mã, tên ..."><button>＋</button>　☐ Tách dòng</div><table class="form-table"><tr><th>STT</th><th>Hàng hóa</th><th>Thương hiệu</th><th>Xuất xứ</th><th>Đơn vị</th><th>Số lượng</th><th>Đơn giá</th><th>Thành tiền</th><th>VAT (%)</th></tr></table></section><section class="form-fields"><div class="photo">▣<small>Ảnh hiện trường</small></div>${field("Mã phiếu","Nhập mã phiếu")}${isIssue?field("Đối tượng nhận *","Chọn người nhận / tổ đội"):field("Nhà cung cấp *","Tìm kiếm nhà cung cấp")}${isIssue?field("Mục đích xuất","Xuất dùng thi công"):field("Đơn hàng mua","Tìm kiếm đơn hàng mua")}${isIssue?field("Hạng mục thi công","Chọn hạng mục thi công"):field("Hợp đồng NCC","Chọn hợp đồng NCC")}${field("Kế hoạch vật tư","Chọn kế hoạch")}${field(isIssue?"Kho xuất *":"Kho nhận *","Kho dự án")}${field(isIssue?"Người lập phiếu *":"Người nhận *","TÀI KHOẢN TRẢI NGHIỆM")}${field(isIssue?"Ngày xuất *":"Ngày nhận *","31/05/2026 21:39")}${isIssue?field("Phiếu nhật ký liên quan","Chọn nhật ký thi công"):field("Người giao","Nhập tên người giao")}${isIssue?field("Ghi chú xuất kho","Nhập ghi chú"):field("Biển số xe","Nhập biển số xe")}</section></div>`);}
...
const contractItemSectionFallback=["KHẢO SÁT - ĐO ĐẠC","CHE PHỦ","PHÁ DỠ","VẬN CHUYỂN","XÂY TRÁT","CHỐNG THẤM","ĐIỆN NƯỚC","PCCC / AN TOÀN KỸ THUẬT","ĐIỀU HÒA","THIẾT BỊ THÔNG MINH - MẠNG - CAMERA","THẠCH CAO","ỐP LÁT","ĐÁ","SƠN BẢ","SÀN GỖ - SÀN NHỰA","CỬA","NHÔM KÍNH","SẮT","GỖ NỘI THẤT","RÈM","CÂY - TIỂU CẢNH","BIỂN BẢNG LOGO","DEFECT CHẤM VÁ","VỆ SINH CN","KHÁC"];
...
let projectCatalogData={constructionCategories:[...contractItemSectionFallback],materialCategories:[...contractMaterialSectionFallback]};
...
function normalizeActiveContractGroup(){
  const groups=[...contractItemSectionOptions(),...contractMaterialSectionOptions()];
  if(!groups.includes(activeContractGroup)){
    activeContractGroup=contractItemSectionOptions()[0]||groups[0]||"KHÁC";
    activeContractSection=contractMaterialSectionOptions().includes(activeContractGroup)?"material":"construction";
  }
}
async function loadProjectCatalog(){
  ...
  normalizeActiveContractGroup();
  return projectCatalogData;
}
function contractKnownCategory(value){
  const category=String(value||"").trim();
  const groups=[...contractItemSectionOptions(),...contractMaterialSectionOptions()];
  return groups.find(group=>group===category)||"KHÁC";
}
...
function contractItemSection(item){return contractKnownCategory(contractItemSectionRaw(item))}
...
function contractQuoteSidebar(){normalizeActiveContractGroup();return `<aside class="contract-quote-side">${contractQuoteSection("HẠNG MỤC THI CÔNG",contractItemSectionOptions(),"data-contract-group","construction")}${contractQuoteSection("HẠNG MỤC VẬT TƯ",contractMaterialSectionOptions(),"data-contract-group","material")}</aside>`}
...
async function contractOwnerView(){await loadProjectCatalog();history.replaceState(null,"",`${location.pathname}?view=contract-owner`);document.querySelector("#project-app").innerHTML=`<section class="contract-owner-page">${contractEstimateFiles()}${contractOwnerFiles()}${contractExtraSection()}${contractSettlementFiles()}</section>`;bindContractOwner();loadContractFiles()}
