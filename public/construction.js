document.head.insertAdjacentHTML("beforeend",'<link rel="stylesheet" href="/construction-overrides.css?v=173">');
...
function normalizeOwnerRequestLabels(scope=document){
  scope.querySelectorAll("button,a,h1,h2,h3,h4,span").forEach((node)=>{
    ...
    if(text==="Thêm yêu cầu CDT")node.textContent="Thêm yêu cầu ghi chú của CDT";
    if(text==="+ Thêm Yêu cầu của CDT")node.textContent="+ Thêm Yêu cầu ghi chú của CDT";
  });
}
...
function organizeProjectOverview(){
  ...
  const zones=[
    ...
    ["progress-zone","03","Ti\u1ebfn \u0111\u1ed9","Th\u1ed1ng k\u00ea nhanh ti\u1ebfn \u0111\u1ed9 thi c\u00f4ng v\u00e0 nhi\u1ec7m v\u1ee5 b\u1ed5 sung trong ng\u00e0y",[".schedule-overview"]],
...
async function render(){
  ...
  <section class="quick box"><h2>Truy cập nhanh</h2><div class="quick-grid quick-grid-three"><div><b>Công nợ</b><a data-view-link="debt-owner">Công nợ CDT ↗</a><a data-view-link="debt-vendor">Công nợ nhà thầu ↗</a><a data-view-link="debt-supplier">Công nợ NCC ↗</a></div><div><b>Công trường</b><a data-form="diary">Thêm nhật ký thi công</a><a data-form="slip-in">Thêm phiếu nhập kho</a><a data-form="expense">Thêm phiếu chi</a><a data-view-link="issues">Thêm vấn đề cần xử lý</a></div><div><b>CDT</b><a data-view-link="rfa">Thêm yêu cầu phê duyệt CDT</a><a data-view-link="owner-request">Thêm yêu cầu ghi chú của CDT</a></div></div></section>
...
function refreshGanttTasks(index){writeGanttStoredState();const body=document.querySelector(".gantt-table tbody");if(body)body.innerHTML=ganttRows();const bars=document.querySelector(".gantt-bars");if(bars)bars.innerHTML=ganttBars(true);if(body||bars){bindGantt();document.querySelector(`[data-gantt-row="${index}"]`)?.classList.add("selected")}}
...
function diaryTaskMaterials(open){const warehouseOptions=["Kho văn phòng","Kho dự án"],itemOptions=diaryWarehouseItemOptions(),defaults=itemOptions.slice(0,2);if(!itemOptions.length)return `<div class="diary-task-table-scroll" data-diary-section="materials" ${open?"":"hidden"}><table class="stat-table diary-edit-table"><thead><tr><th>STT</th><th>Thiết bị Vật tư</th><th>Kho xuất</th><th>Thời gian xuất</th><th>Số lượng</th><th>Đơn vị</th><th></th></tr></thead><tbody><tr class="diary-empty-row"><td colspan="7">Chưa có vật tư trong danh sách Kho. <a data-view-link="warehouse">Mở danh sách Kho</a></td></tr></tbody></table></div>`;const rows=defaults.map((item,index)=>[String(index+1),diaryEditSelect(item,itemOptions),diaryEditSelect("Kho dự án",warehouseOptions),diaryEditInput("18:00",'type="time"'),diaryEditInput(index===0?"23":"45",'inputmode="decimal"'),diaryEditSelect(diaryWarehouseUnitForItem(item),diaryMaterialUnitOptions),diaryDeleteRowButton("materials")]);return `<div class="diary-task-table-scroll" data-diary-section="materials" ${open?"":"hidden"}>${diaryEditableTable(["STT","Thiết bị Vật tư","Kho xuất","Thời gian xuất","Số lượng","Đơn vị",""],rows)}</div>`}
...
function diaryAddedRow(kind,index){if(kind==="materials"){const itemOptions=diaryWarehouseItemOptions(),item=itemOptions[0]||"";return `<tr><td>${index}</td><td>${itemOptions.length?diaryEditSelect(item,itemOptions):'<em>Chưa có vật tư trong Kho</em>'}</td><td>${diaryEditSelect("Kho dự án",["Kho văn phòng","Kho dự án"])}</td><td>${diaryEditInput("18:00",'type="time"')}</td><td>${diaryEditInput("",'inputmode="decimal"')}</td><td>${diaryEditSelect(diaryWarehouseUnitForItem(item),diaryMaterialUnitOptions)}</td><td>${diaryDeleteRowButton("materials")}</td></tr>`}return `<tr><td>${diaryEditInput(`Ca ${index}`)}</td><td>${diaryEditInput("",'inputmode="numeric"')}</td><td>${diaryEditInput("07:00",'type="time"')}</td><td>${diaryEditInput("18:00",'type="time"')}</td><td>${diaryDeleteRowButton("labor")}</td></tr>`}
...
function requestCreateAction(view,title){if(view==="rfa")return approvalRequestFormView();if(view==="owner-request")return ownerRequestFormView();if(view==="issues")return issueFormView();return alert(`Tạo mới ${title} sẽ được lưu trong dự án ${currentProject.name}.`)}
